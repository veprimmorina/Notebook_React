...
const categorize = () => {
  ...
  {school.map((note)=> <><tr className='text-dark border-bottom'><td>{note.title}</td><td className=''>{note.description}</td>
  <td>{note.noteData}</td></tr></>)}
  </table>}
 ...
 {metting.map((note)=> <><tr className='text-dark border-bottom'><td>{note.title}</td><td className=''>{note.description}</td><td>{note.noteData}</td></tr></>)}
  </table>}
 ...
 {other.map((note)=> <> <tr className='text-dark border-bottom'><td>{note.title}</td><td className=''>{note.description}</td><td>{note.noteData}</td><td>{note.category}</td></tr></>)}
  </table>}
  </>:<><table className='table table-dark shadow'>
  <thead className='thead-light'><tr><th>Title</th><th>Description</th><th>Date:</th></tr></thead>
   {notes.map((note)=>(note.category==search ? <tr className='text-dark border-bottom'><td>{note.title}</td><td>{note.description}</td><td>{note.noteData}</td></tr> : ""))}</table></>}
    </Card.Body>
    </Card>
  )
}
